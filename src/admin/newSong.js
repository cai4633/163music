  ...
  let controller = {
    ...
    bindEvents() {
      ...
      $("#uploadInput").on("change", (e) => {
        this.model.files = Array.from($("#uploadInput")[0].files)
        let description = this.model.files.length ? `已选择${this.model.files.length}个文件` : "请选择文件..."
        $(".text").text(description)
      })
      $("#chooseFiles").on("drop", (e) => {
        e.preventDefault()
        this.model.files = [].concat(Array.from(e.originalEvent.dataTransfer.files))
        let description = this.model.files.length ? `已选择${this.model.files.length}个文件` : "请选择文件..."
        $(".text").text(description)
      })
      //多文件上传
